 ...
 */
static int ov7740_s_stream(struct v4l2_subdev *sd, int enable)    /*set data stream  ?? */
{
	return 0;
}
...
static int ov7740_g_chip_ident(struct v4l2_subdev *sd,
			       struct v4l2_dbg_chip_ident *id)            /*get device id*/
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int ov7740_g_register(struct v4l2_subdev *sd,
			     struct v4l2_dbg_register *reg)              /*get register value*/
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
...
static int ov7740_s_register(struct v4l2_subdev *sd,
			     const struct v4l2_dbg_register *reg)            /*set register value*/
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int ov7740_g_fmt(struct v4l2_subdev *sd,
			struct v4l2_mbus_framefmt *mf)   /*get video format*/
{
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct ov7740_priv *priv = to_ov7740(client);
			...
			struct v4l2_mbus_framefmt *mf)
{
	...
	if(mf->code == V4L2_MBUS_FMT_YUYV8_2X8)
	{
		printk("<0>" "mf->code = V4L2_MBUS_FMT_YUYV8_2X8\n");
	}
	if(mf->colorspace == V4L2_COLORSPACE_JPEG)
	{
		printk("<0>" "mf->colorspace = V4L2_COLORSPACE_JPEG\n");
	}	
...
static int ov7740_enum_fmt(struct v4l2_subdev *sd, unsigned int index,           /*enum video format*/
			   enum v4l2_mbus_pixelcode *code)
{
